  ...
  return (
    <div style={styles.document}>
      ...
      <div style={styles.contactInfo}>
        ...
        <div>Website: <a href="http://vintageTimepiece.com">http://vintageTimepiece.com</a></div>
        <div>Email: vintageTimepiece@gmail.com</div>
      </div>
      ...
      <div style={styles.section}>
        <div style={styles.productInfo}>
          <div><span style={styles.left}>Brands:</span> {brand}</div>
          <div><span style={styles.left}>Description:</span> {description}</div>
          <div><span style={styles.left}>Type:</span> {type}</div>
          <div><span style={styles.left}>Dial Color:</span> {dialColor}</div>
          <div><span style={styles.left}>Box:</span> {box ? 'YES' : 'NO'}</div>
          <div><span style={styles.left}>Paper:</span> {paper ? 'YES' : 'NO'}</div>
          <div><span style={styles.left}>Water Resistance:</span> {waterResistance}</div>
          <div><span style={styles.left}>Case Material:</span> {caseMaterial}</div>
          <div><span style={styles.left}>Case Size:</span> {caseSize}</div>
          <div><span style={styles.left}>Past Usage Time:</span> {pastUsageTime}</div>
          <div><span style={styles.left}>Year Of Product:</span> {yearOfProduction}</div>
          <div><span style={styles.left}>Remaining Insurance:</span> {remainingInsurance}</div>
        </div>
      ...
      <div style={styles.footer}>
        <div style={styles.center}><strong>VT WATCH INSPECTION CENTER</strong></div>
        <div style={styles.center}><strong>CERTIFICATE OF AUTHENTICITY</strong></div>
        <div style= {styles.details}>This certificate is independently issued by VT Watch Inspection Center and is not authorized by any watch brands.
   It is provided based on the inspection results and evaluations conducted by our senior experts with extensive experience.</div>

        <div style={styles.detailsContainer}>
          <div style={styles.details}>
            <div style={styles.left}>Certification Details:</div>
            <div style={styles.left2}>Issuance Date: January 7, 2003</div>
